...
if analysis_sensibility_only == False:
    ...
    share_jetfuel_ft = [i for i in range(40, 110, 10)]
    for case_number in range(0, 638):
        ...
        #config['case_studies'] = os.path.join(project_path,'case_studies', 'paper1', 'ft_analysis')
        config['case_study'] = 'scenario_' + str(case_number+1)#'jetfuel_RF_EU_ft'+str(share_jetfuel_ft[case_number])#
        ...
        """config['all_data']['Layers_in_out'].loc['REFINERY_JETFUEL', 'JETFUEL'] = share_jetfuel_ft[case_number]/100
        config['all_data']['Layers_in_out'].loc['REFINERY_JETFUEL', 'GASOLINE'] = (100-share_jetfuel_ft[case_number])/200
        config['all_data']['Layers_in_out'].loc['REFINERY_JETFUEL', 'DIESEL'] = (100-share_jetfuel_ft[case_number])/200"""
